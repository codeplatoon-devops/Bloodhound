...
def handle_slack_command_http(event: dict[str, Any]) -> dict[str, Any]:
    ...
    # ------------------------------------------------------------
    # System status command (/v2_status)
    #
    # DEFERRED during the PR #2 merge: this routes to worker mode
    # "status", which is implemented by PR #2's status_service (part of
    # the handlers/services architecture we did not adopt). Our worker
    # (bloodhound.app.run) has no "status" mode, so wiring this up is a
    # follow-up — see docs/INTEGRATION_PR2_NOTES.md. Left unrouted rather
    # than silently running a full scan under the /v2_status name.
    # ------------------------------------------------------------
    ...
    # Destructive teardown command (/v2_seek_destroy is the PR #2 alias)
